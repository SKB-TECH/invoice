...
import { contractResponseToListRow } from "@/lib/contrats/contrat-api-mapper";
import type { ContratStatutUi } from "@/lib/contrats/contrats-data";
...
export default function ContratsPage() {
    ...
    const { data, isPending, isError } = useContracts({ per_page: 100 });

    const rows = useMemo(
        () => data?.items.map(contractResponseToListRow) ?? [],
        [data?.items]
    );
