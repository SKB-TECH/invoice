import type {
    InvoiceNormalizationPreviewContent,
...
type BuildOptions = {
    ...
    rows: InvoiceNormalizationReportApiRow[];
...
function resolveClientName(
    ...
    const directName = pickString(row, ["client_name", "customer_name", "legal_name"]);
    if (directName !== "—") return directName;

    const clientId = pickString(row, ["client_id"]);
    const client = clients.find(
        (item) => String(item.id) === clientId || String(item.client_id ?? "") === clientId,
    );
...
export function buildInvoiceNormalizationPreviewDisplay({
    filters,
    rows,
    profile,
    user,
    ...
    const content: InvoiceNormalizationPreviewContent = {
    ...
    return { variant: "invoice-normalization", content };
}
